...
def _analyze(yaml_text: str):
    """Validate/expand YAML text directly (no file path required)."""
    try:
    ...
    # Build resolver against the current working directory.
    base_dir = os.getcwd()
    resolver, lib_errors = boardspec.build_resolver(spec, base_dir, [])
...
def validate(yaml_text: str) -> dict:
    _, errors, warnings = _analyze(yaml_text)
    return {"ok": not errors, "errors": errors, "warnings": warnings}


def expand(yaml_text: str) -> dict:
    expanded, errors, warnings = _analyze(yaml_text)
    if errors:
...
def export(yaml_text: str, target: str) -> dict:
    ...
    return {"ok": True, "errors": errors, "warnings": warnings, "content": content}
